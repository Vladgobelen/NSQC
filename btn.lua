...
function vybor:configure(id)
	...
	vybor[id]:SetScript("OnEnter",function(self)
		...
		if mioFld1 ~= nil then
			...
			if mioFld1[nome]["объекты"][tostring(testQ["idp"])] == "zt" and id == 3 and tonumber(testQ["brevna"]) < 15 then
				GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
	...
	SendAddonMessage("pN1","", "guild")
		...
		if mioFld1 ~= nil then
			...
			if nome == myNome then
				if mioFld1[nome]["объекты"][tostring(testQ["idp"])] == "zt" and id == 4 then
				...
				if mioFld1[nome]["объекты"][tostring(testQ["idp"])] == "zt" and id == 2 then
			...
			if testQ["bbConfig"] == nil then
				if mioFld1[nome]["объекты"][tostring(testQ["idp"])] == "z" and id == 6 then
					...
					elseif testQ["temp"] == 1 then
						...
						testQ["brevna"] = tonumber(testQ["brevna"]) - 10
						testQ["nikQB"] = antc(testQ["brevna"])
						SendAddonMessage("ns_hs " .. testQ["idp"], nome, "guild")
						for i=1,100 do
							if vybor[i] ~= nil then
								vybor[i]:Hide()
							end
...
-- 				else
-- 					print("Я беспорядочно стучу руками по дереву...зачем? Нет, правда, нахрена?")
-- 				end
			--end
			end
-- 			if mioFld1[nome]["объекты"][tostring(id)] == "m" then
...
--[[
		...
		if mioFld1 ~= nil then
			if mioFld1[nome] ~= nil then
				SendAddonMessage("ns_shMc " .. id, nome, "guild")
	...
	self[id]:SetScript("OnEnter",function(self, button)
		...
		if testQ["domZ"] == "muzeum" then
			...
			if mioFld1[nome]["muzeum"][4] == "1" then
				...
				if id == 4 then
					GameTooltip:AddLine("|cffDC143CНужно срубить 10 деревьев")
				end
			end
		end
